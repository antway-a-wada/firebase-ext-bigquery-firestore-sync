...
describe('queryUpdatedRecords', () => {
  ...
  it('should query records without timestamp filter on initial sync', async () => {
    ...
    const mockGetMetadata = jest
      .fn()
      .mockResolvedValue([{statistics: {query: {totalBytesProcessed: '1024'}}}])
    const mockJob = {
      id: 'job123',
  ...
  it('should query records with timestamp filter', async () => {
    ...
    const mockGetQueryResults = jest
      .fn()
      .mockResolvedValue([[{id: '2', name: 'User 2', updated_at: '2024-01-02T00:00:00Z'}]])
    const mockGetMetadata = jest
      .fn()
      .mockResolvedValue([{statistics: {query: {totalBytesProcessed: '512'}}}])
    const mockJob = {
      id: 'job123',
      getQueryResults: mockGetQueryResults,
      getMetadata: mockGetMetadata,
...
describe('queryAllDocumentIds', () => {
  it('should return all document IDs', async () => {
    ...
    const mockGetQueryResults = jest.fn().mockResolvedValue([[{id: '1'}, {id: '2'}, {id: '3'}]])
    const mockJob = {
      getQueryResults: mockGetQueryResults,
    }
    const mockCreateQueryJob = jest.fn().mockResolvedValue([mockJob])
...
describe('getMaxTimestamp', () => {
  it('should return the maximum timestamp', async () => {
    ...
    const mockGetQueryResults = jest
      .fn()
      .mockResolvedValue([[{max_timestamp: {value: '2024-01-15T12:00:00.000Z'}}]])
    const mockJob = {
      getQueryResults: mockGetQueryResults,
    }
    const mockCreateQueryJob = jest.fn().mockResolvedValue([mockJob])
